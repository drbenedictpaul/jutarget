# --- HARDWARE AUTHENTICATION (Multi-License) ---

# List of authorized machine ID pairs (UUID, Baseboard)
const BLESSED_MACHINES = [
    # Friend's Machine 1
    ("4c4c4544-0053-4d10-8051-b6c04f424e33", "/6SMQBN3/CNPE10022F0BP4/"),
    
    # Friend's Machine 2
    ("4c4c4544-0048-5a10-8052-c6c04f505433", "/FHZRPT3/CNFCW0028U00G4/"),

    # Developer's Machine (Fedora) - Remember to replace the placeholder
    ("7c0ce48b-275f-11e3-aa05-28d2442e1ee3", "/1005943800730/YB01410308/")
]

# Get IDs passed from the C launcher
...
# Secure check against the list of blessed machines
...
route("/print-report/:folder") do
    ...
    rd_file = joinpath(target_path, "rd_analyzer_report.txt")
    drug_rows = ""
    if isfile(drug_file); df = CSV.read(drug_file, DataFrame); for row in eachrow(df); color = row.Prediction == "RESISTANT" ? "#d32f2f" : "#388e3c"; drug_rows *= "<tr><td>$(row.Drug)</td><td style='color:$color; font-weight:bold;'>$(row.Prediction)</td><td>$(row.Evidence)</td><td>$(row.Mechanism)</td></tr>"; end; end
